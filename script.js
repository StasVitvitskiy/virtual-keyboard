...
wrapper.addEventListener('click', (event) => {
  ...
  if (target.innerHTML.trim().toLowerCase() === 'del') {
    ...
    textArea.value = textArea.value.split('').filter((el, i) => i !== textArea.selectionStart).join('');
  ...
  if (target.innerHTML.trim().toLowerCase() === 'backspace') {
    const cursorPosition = textArea.selectionStart;
    textArea.value = textArea.value.split('').filter((el, i) => i !== textArea.selectionStart - 1).join('');
...
document.addEventListener('keydown', (event) => {
  ...
  if (htmlElements.length === 0) {
    htmlElements = document.querySelectorAll(`[data-control="${key.toLowerCase()}"]`);
  }
